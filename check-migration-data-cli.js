/**
 * Real Data Check Script (Using Firebase Emulator/CLI)
 * Queries actual Firebase RTDB and Firestore to determine migration needs
 * 
 * Run: node check-migration-data-cli.js
 */
...
// Firebase config - uses environment variables from .env
...
async function checkMigrationData() {
  ...
  try {
    ...
    console.log('📦 Fetching all devices from RTDB...');
...
async function checkDevice(deviceId, deviceData) {
  ...
  try {
    const analyticsCol = collection(db, 'device_analytics');
    const q = query(analyticsCol, where('deviceId', '==', deviceId));
    const analyticsQuery = await getDocs(q);
...
async function checkActivityLogs() {
  try {
    ...
    const q = query(logsCol, where('outputId', 'in', oldOutputIds), limit(100));
    const logsQuery = await getDocs(q);
    ...
    const newQ = query(logsCol, where('outputId', 'in', newOutputIds), limit(10));
    const newLogsQuery = await getDocs(newQ);
